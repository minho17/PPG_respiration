...
def main():
    ...
    for ind_te in range(start_sub,n_sub):
        ...
        model = build_model.Correncoder_model2( [8,8,8], [150,75,50], [20,20,10], 0.5).to(device)
        ...
        tr_data = dataset2(ind_tr, data, win_anal, win_move,fs)
        val_data = dataset2(ind_val, data, win_anal, win_move,fs)
        te_data = dataset2(ind_te, data, win_anal, win_move,fs)
        ...
        for i_epoc in range(n_epoc):       
            ...
            for batch, (sig,label,label_f) in pbar:
                x = sig.to(torch.float32).to(device)
                y = label.to(torch.float32).to(device)
                y2 = label_f.to(torch.float32).to(device)
